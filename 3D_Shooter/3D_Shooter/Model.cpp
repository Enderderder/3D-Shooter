...
void CModel::Render(CCamera * camera, GLuint _program)
{
	for (GLuint i = 0; i < this->meshes.size(); i++) {

		//printf("mesh size: %d \n", meshes.size());

		this->meshes[i].Render(camera, _program);
	}
}
...
ModelMesh CModel::processMesh(aiMesh * mesh, const aiScene * scene)
{
	// Data to fill
...
GLint CModel::TextureFromFile(const char * path, string directory)
{
	...
	// Unbind the TextureID and free the image data
